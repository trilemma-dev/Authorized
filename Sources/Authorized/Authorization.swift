...
///
/// ### Rights
/// - ``requestRights(_:environment:options:)-5rm4u``
/// - ``requestRights(_:environment:options:callback:)``
/// - ``requestRights(_:environment:options:)-817qz``
...
/// - ``init(from:)``
/// - ``encode(to:)``
public class Authorization: Decodable {
    
    /// The underlying [`AuthorizationRef`](https://developer.apple.com/documentation/security/authorizationref).
    ///
    ...
    ///
    /// To request rights, use ``Authorization/requestRights(_:environment:options:)-817qz`` or an asynchronous equivalent.
    public init() throws {
    ...
    private static func deserialize(from serialization: Data) throws -> AuthorizationRef {
        ...
        if serialization.count != externalFormChildrenCount {
    ...
    public static func checkRights(
        _ rights: Set<AuthorizationRight>,
    ...
    ///
    /// See the discussion for  ``Authorization/requestRights(_:environment:options:)-817qz`` This function behaves similarly, except that it
    ...
    public func requestRights(
        ...
        environment: Set<AuthorizationEnvironmentEntry>,
        options: Set<AuthorizationOption>
    ) async throws -> [AuthorizationRight] {
...
extension Authorization: Encodable {
