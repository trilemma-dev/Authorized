...
/// ``Authorization/requestRights(_:environment:options:)-817qz`` or an asynchronous equivalent then this right was defined at the moment
...
public struct AuthorizationRight: AuthorizationElement {
    ...
    ///
    /// This only has defined behavior when the right was returned from  ``Authorization/requestRights(_:environment:options:)-817qz`` or
    /// asynchronous equivalents. Otherwise this property's value is undefined.
    public var cannotPreAuthorize: Bool {
        return self.flags == kAuthorizationFlagCanNotPreAuthorize
    }
}

extension AuthorizationRight: Hashable { }
...
/// - ``forPrompt(message:)``
/// - ``forIcon(_:)``
public struct AuthorizationEnvironmentEntry: AuthorizationElement {
...
extension AuthorizationEnvironmentEntry: Hashable { }
...
internal extension Set where Element: AuthorizationElement {
    ...
    private static func convertRecursively<Result>(
        wrappers: [AuthorizationElement],
        ...
        } else { // Reached the base case, create the AuthorizationItemSet with all of the converted items
